...
def test_process_parser_defaults_to_silicon_pyannote_path(tmp_path: Path) -> None:
...
def test_process_parser_accepts_hidden_mlx_backend(tmp_path: Path) -> None:
    args = cli.build_parser().parse_args(
        [
            "process",
            str(tmp_path / "a.mp4"),
    ...
    assert args.asr_backend == "kotoba-mlx"
    assert args.mlx_model_path == "/tmp/model"
    assert args.mlx_device == "cpu"
    assert args.mlx_word_timestamps is True
...
def test_qwen_backend_reports_missing_experiment_environment(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "uv sync --group torch --group pyannote --group qwen" in captured.out
