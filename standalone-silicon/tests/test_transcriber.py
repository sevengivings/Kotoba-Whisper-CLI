...
from kotoba_standalone.transcriber import KotobaTranscriber, TranscriptionDependencyError, read_wav_mono_float32_segment
...
def test_read_wav_mono_float32_segment_reads_only_requested_frames(tmp_path: Path) -> None:
    ...
    audio = read_wav_mono_float32_segment(str(wav_path), 2.0, 3.0)

    assert audio["sampling_rate"] == 100
    assert len(audio["array"]) == 100
    assert audio["array"][0] == pytest.approx(200 / 32768.0)
    assert audio["array"][-1] == pytest.approx(299 / 32768.0)
