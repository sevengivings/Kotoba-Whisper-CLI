...
from kotoba_standalone.launcher import (
    ...
    default_model_device_for_backend,
    dropped_paths_from_event,
    effective_model_device,
    estimate_work_text,
...
class _FakeTk:
    def splitlist(self, data: str) -> tuple[str, ...]:
        return ("/Users/me/Video Clips/a sample.wmv", "/Users/me/Video Clips/second.mp4")
...
def test_build_process_command_defaults_to_pyannote_vad(tmp_path: Path) -> None:
...
def test_build_process_command_adds_kotoba_backend_for_mps(tmp_path: Path) -> None:
    command = build_process_command(
        LauncherOptions(
            input_path=tmp_path / "sample.mp4",
            output_dir=tmp_path / "out",
    ...
    assert command[command.index("--asr-backend") + 1] == "kotoba"
    assert command[command.index("--model-device") + 1] == "mps"


def test_build_process_command_keeps_mps_for_mlx_vad(tmp_path: Path) -> None:
    command = build_process_command(
        LauncherOptions(
            input_path=tmp_path / "sample.mp4",
            output_dir=tmp_path / "out",
    ...
    assert command[command.index("--asr-backend") + 1] == "kotoba-mlx"
    assert command[command.index("--model-device") + 1] == "mps"


def test_build_process_command_forces_cpu_for_faster_backend(tmp_path: Path) -> None:
    command = build_process_command(
        LauncherOptions(
            input_path=tmp_path / "sample.mp4",
            output_dir=tmp_path / "out",
    ...
    assert command[command.index("--model-device") + 1] == "cpu"


def test_effective_model_device_keeps_mps_for_gpu_capable_backends() -> None:
    assert effective_model_device("faster-kotoba", "mps") == "cpu"
    assert effective_model_device("kotoba-mlx", "mps") == "mps"
    assert effective_model_device("kotoba", "mps") == "mps"


def test_default_model_device_follows_selected_backend() -> None:
    assert default_model_device_for_backend("faster-kotoba", ("mps", "cpu")) == "cpu"
    assert default_model_device_for_backend("kotoba-mlx", ("mps", "cpu")) == "mps"
    assert default_model_device_for_backend("kotoba", ("mps", "cpu")) == "mps"
    assert default_model_device_for_backend("kotoba", ("cpu",)) == "cpu"
    assert default_model_device_for_backend("qwen3", ("cuda:0", "cpu")) == "cuda:0"
...
def test_available_model_devices_lists_mps_device(monkeypatch) -> None:
    fake_torch = types.SimpleNamespace(
    ...
    assert available_model_devices() == ("mps", "cpu")
...
def test_dropped_paths_from_event_uses_tk_splitlist_for_spaced_paths() -> None:
    paths = dropped_paths_from_event(_FakeRoot(), "{/Users/me/Video Clips/a sample.wmv} {/Users/me/Video Clips/second.mp4}")

    assert paths == [Path("/Users/me/Video Clips/a sample.wmv"), Path("/Users/me/Video Clips/second.mp4")]
