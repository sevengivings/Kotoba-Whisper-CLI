    ...
    def load(self) -> None:
        try:
            ...
            raise TranscriptionDependencyError(
                ...
                "Run 'uv sync --group transcribe --group torch', then retry."
            ) from exc
        ...
        if model_device.startswith("cuda:"):
            ...
            if not _mps_available(torch):
                raise TranscriptionDependencyError(
                    "Selected processing device is mps, but the PyTorch MPS backend is not available. "
                    "Choose 'cpu' in the processing device setting, then retry."
                )
...
def read_wav_mono_float32(wav_path: str) -> dict[str, Any]:
    return read_wav_mono_float32_segment(wav_path)


def read_wav_mono_float32_segment(wav_path: str, start_s: float = 0.0, end_s: float | None = None) -> dict[str, Any]:
    import numpy as np
    ...
    with wave.open(wav_path, "rb") as wav_file:
        ...
        start_frame = max(0, min(total_frames, int(round(start_s * sample_rate))))
        end_frame = total_frames if end_s is None else max(start_frame, min(total_frames, int(round(end_s * sample_rate))))
