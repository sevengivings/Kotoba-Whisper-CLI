...
try:
    ...
    BooleanVar = Listbox = StringVar = Tk = Toplevel = ScrolledText = None  # type: ignore[assignment]
...
def build_process_command(options: LauncherOptions) -> list[str]:
    model_device = effective_model_device(options.asr_backend, options.model_device)
    command = [
        sys.executable,
        "-m",
    ...
    if options.asr_backend == "kotoba":
        command.extend(["--asr-backend", "kotoba"])
...
def effective_model_device(asr_backend: str, requested_device: str) -> str:
    if asr_backend == "faster-kotoba":
        return "cpu"
    return requested_device.strip() or "cpu"


def default_model_device_for_backend(asr_backend: str, available_devices: tuple[str, ...]) -> str:
    if asr_backend in {"kotoba", "kotoba-mlx"} and "mps" in available_devices:
        return "mps"
    if asr_backend == "qwen3" and any(device.startswith("cuda:") for device in available_devices):
        return next(device for device in available_devices if device.startswith("cuda:"))
    return "cpu"
...
def available_model_devices() -> tuple[str, ...]:
    ...
    try:
        ...
        elif _torch_mps_available(torch):
            devices = ["mps"] + devices
    except Exception:
        return tuple(devices)
    return tuple(devices)
...
def estimate_history_text(output_dir: Path, asr_backend: str = "faster-kotoba") -> str:
...
def _process_history_ratios(output_dir: Path, asr_backend: str | None = "faster-kotoba") -> list[float]:
    ratios: list[float] = []
    ...
    for metadata_path in sorted(output_dir.glob("*.process.json"), key=lambda path: path.stat().st_mtime):
        ...
        if asr_backend and str(data.get("asr_backend") or "faster-kotoba") != asr_backend:
            continue
    ...
    def start(self) -> None:
        ...
        if asr_backend == "qwen3" and not qwen_environment_python().exists():
            messagebox.showwarning(
                "Qwen3-ASR 설치 필요",
                "Qwen3-ASR을 사용하려면 먼저 .venv-qwen 환경을 설치해 주세요.",
            )
...
def main() -> int:
    if Tk is None:
        print("Tkinter is not available in this Python installation. Install a Python build with Tk support, then retry.")
        return 1
    root = Tk()
