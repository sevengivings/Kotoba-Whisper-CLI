...
def build_parser() -> argparse.ArgumentParser:
    ...
    process.add_argument("--model-device", default="cpu")
    process.add_argument("--model-dtype", default="float32")
    ...
    align.add_argument("--model-device", default="cpu")
...
def maybe_reexec_with_qwen_environment(argv: list[str] | None) -> int | None:
    ...
    if qwen_python is None:
        print_error(
            ...
            "  Install it first:\n"
            "    UV_PROJECT_ENVIRONMENT=.venv-qwen uv sync --group torch --group pyannote --group qwen\n"
