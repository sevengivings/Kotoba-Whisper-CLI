...
def test_config_loading() -> None:
    ...
    config = load_config(docker_root / "config" / "config.yaml")
    ...
    assert config.inference.vad_engine == "pyannote"
    assert config.inference.pyannote_model == "pyannote/segmentation-3.0"
    ...
    assert config.inference.fallback_subtitle_start_delay_s == 0.5
    assert config.inference.filter_short_repeated_phrases is True
    assert "\u3054\u3081\u3093\u3002" in config.inference.filtered_always_phrases
    assert "\u3042\u308a\u304c\u3068\u3046\u3054\u3056\u3044\u307e\u3057\u305f\u3002" in config.inference.filtered_always_phrases
    assert "\u3054\u3081\u3093\u3002" not in config.inference.filtered_short_phrases
    assert "\u3042\u308a\u304c\u3068\u3046\u3054\u3056\u3044\u307e\u3057\u305f\u3002" not in config.inference.filtered_short_phrases
