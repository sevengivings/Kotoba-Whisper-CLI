    ...
    try:
        ...
        app_config = AppConfig(
            ...
            inference=InferenceConfig(
                ...
                vad_engine=str(inference.get("vad_engine", "pyannote")),
                pyannote_model=str(inference.get("pyannote_model", "pyannote/segmentation-3.0")),
...
def validate_config(config: AppConfig) -> None:
    ...
    if config.inference.vad_engine not in {"ffmpeg", "pyannote"}:
        raise ValueError("inference.vad_engine must be 'ffmpeg' or 'pyannote'")
    if not config.inference.pyannote_model:
        raise ValueError("inference.pyannote_model must not be empty")
    ...
    if config.inference.fallback_subtitle_max_duration_s <= 0:
        raise ValueError("inference.fallback_subtitle_max_duration_s must be > 0")
    if config.inference.fallback_subtitle_chars_per_second <= 0:
