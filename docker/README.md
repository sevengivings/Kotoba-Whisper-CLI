# Kotoba-Whisper Docker Edition

[프로젝트 홈](../README.md) | [English summary](../README.en.md)

이 문서는 기존 `input` 폴더 감시 방식의 Docker 버전을 설명합니다. 새로 설치한다면 Docker가 필요 없는 [standalone 버전](../standalone/README.md)을 먼저 권장합니다.

Docker 코드, 실행 스크립트, 설정, 데이터 폴더와 모델 캐시는 모두 이 `docker/` 폴더에 있습니다.

- Windows Docker 실행 위치: `C:\Python\Kotoba-Whisper-CLI\docker`
- Ubuntu Docker 실행 위치: `~/Kotoba-Whisper-CLI/docker`
...
- 결과 파일 생성:
  ...
  - `output/<name>.vad.json` (pyannote 검출 구간)
- 처리 완료 원본은 `archive`, 실패 원본은 `failed`로 이동
- 번들된 pyannote VAD로 사람 음성 구간을 검출해 긴 자막 뭉침과 배경음 오탐 완화
...
- Windows 권장 실행 위치: `C:\Python\Kotoba-Whisper-CLI\docker`
- Ubuntu 예시 실행 위치: `~/Kotoba-Whisper-CLI/docker`
...
기본 `pyannote/segmentation-3.0` 모델은 MIT 라이선스에 따라 Docker 이미지에도 포함됩니다. 원본 라이선스, 모델 카드, 고정 revision과 checksum은 `app/models/pyannote-segmentation-3.0`에 보존되어 있으며 Hugging Face 로그인은 필요하지 않습니다.

## 시작

Docker watcher를 계속 사용하는 경우 이 `docker/` 폴더에서 명령을 실행합니다. 새 사용자를 위한 기본 버전은 저장소 루트의 `standalone/`입니다.

```powershell
cd C:\Python\Kotoba-Whisper-CLI\docker
docker compose up -d
```
...
기본 pyannote 대신 특정 파일만 기존 FFmpeg 무음 VAD로 처리하려면:
...
특정 파일만 FFmpeg VAD로 처리하면서 전체 음량 분포로 무음 기준을 자동 선택하려면:
...
디렉터리 안의 파일 전체를 FFmpeg VAD로 처리하며 같은 무음 기준을 적용하려면:
...
디렉터리 안의 각 파일을 FFmpeg VAD로 처리하며 무음 기준을 자동 선택하려면:
...
- `vad_engine`: 기본값 `pyannote`는 사람 음성 구간을 모델로 검출합니다. 기존 음량 기반 방식을 계속 쓰려면 `ffmpeg`로 바꿉니다.
- `pyannote_model`: 기본 번들 모델 ID입니다. Docker 이미지에 MIT 원본 가중치가 포함되므로 Hugging Face 계정이나 토큰이 필요하지 않습니다.
...
아래 항목은 `vad_engine: ffmpeg`일 때만 사용합니다.
...
- `fallback_subtitle_start_delay_s`: FFmpeg VAD의 fallback 자막 전체를 뒤로 미는 시간입니다. pyannote에서는 적용하지 않습니다.

FFmpeg 자동 기준을 사용하면 처리 결과 JSON에 다음처럼 분석값이 남습니다.
...
`threshold_db`가 기본값보다 더 낮게 자주 붙으면 해당 영상들이 전체적으로 작은 음성 위주라는 뜻입니다. 일반적으로는 이 값을 조정하기보다 기본 pyannote를 사용하는 편이 배경 음악과 사람 음성을 구분하는 데 유리합니다.
...
잡음이나 숨소리가 `ごめん。` 같은 문구로 잘못 전사되는 경우가 있습니다. 이를 줄이기 위해 짧은 단독 문구 필터가 있습니다.
